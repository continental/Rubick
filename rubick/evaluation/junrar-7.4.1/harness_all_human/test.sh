...
then
	...
	../../jazzer/jazzer_driver --cp=${class_path} --target_class=${class_name} --jvm_args="-Xmx2048m" --reproducer_path=/dev/null -max_total_time=$SECONDS -close_fd_mask=3 --instrumentation_excludes=net.automatalib.**:ahg.**:org.json.simple.** -rss_limit_mb=10240 ./seeds 
	#../../jazzer/jazzer_driver --cp=${class_path} --target_class=${class_name} --jvm_args="-Xmx2048m" --reproducer_path=/dev/null -max_total_time=$SECONDS --instrumentation_excludes=net.automatalib.**:ahg.**:org.json.simple.** ./seeds 
else
	# dry run test
