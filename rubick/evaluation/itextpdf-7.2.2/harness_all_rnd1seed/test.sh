...
then
	...
	../../jazzer/jazzer_driver --cp=${class_path} --target_class=${class_name} --jvm_args="-Xmx4096m" --reproducer_path=/dev/null -max_total_time=$SECONDS -close_fd_mask=3 -rss_limit_mb=10240 --instrumentation_excludes=net.automatalib.**:ahg.**:org.json.simple.** ./seeds 
else
	# dry run test
	../../jazzer/jazzer_driver --cp=${class_path} --target_class=${class_name} --jvm_args="-Xmx4096m" --reproducer_path=/dev/null -runs=2 --instrumentation_excludes=net.automatalib.**:ahg.**:org.json.simple.**:com.fuzz.** ./seeds 
fi
